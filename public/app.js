...
function protectionVoucherCard(voucher) {
  ...
  const pixDelivery = voucher.status === "issued"
    ? `<div class="center-pix-offramp"><div><b>BENEFIT DELIVERY · PIX SANDBOX</b><span>Simulate delivery of this eligible benefit in BRL through the Pix test flow.</span></div><button type="button" data-center-pix-redeem="${voucher.id}" data-voucher-code="${voucher.code}" data-voucher-type="${voucher.type}">Simulate benefit via Pix</button><small>This is not a card load or a real Pix transfer. No real BRL moves.</small></div>`
    : "";
  return `<article class="center-voucher">
    ...
    <div class="center-voucher-proof"><b>Audit hash · ${voucher.auditReceipt?.algorithm || "SHA-256"}</b><code>${voucher.auditReceipt?.hash || "Hash pending"}</code>${voucher.settlement?.transactionHash ? `<b>Voucher funding · ${voucher.settlement.amount} ${voucher.settlement.asset} · Stellar Testnet</b><a href="${voucher.settlement.explorerUrl}" target="_blank" rel="noopener noreferrer"><code>${voucher.settlement.transactionHash}</code><span>Open in Stellar Expert ↗</span></a>` : `<b>Voucher funding · TESTNET FUNDING PENDING</b>`}<b>Card load · NOT CONFIGURED</b><small>A card-voucher requires an issuer or card processor integration. Pix is a separate benefit-delivery rail.</small></div>
    ${pixDelivery}
    ${voucher.pixSettlement ? `<div class="pix-proof"><b>PIX SANDBOX · SIMULATED PAID</b><span>${voucher.pixSettlement.merchant.name}</span><code>${voucher.pixSettlement.endToEndId}</code>${pixPaidAt ? `<span><b>Simulated at</b> ${pixPaidAt}</span>` : ""}${voucher.pixSettlement.pixRequest?.payloadDigest ? `<small>Pix Copia e Cola · SHA-256</small><code>${voucher.pixSettlement.pixRequest.payloadDigest}</code>` : ""}<small>R$ ${voucher.pixSettlement.payout.amount} simulated · no real BRL moved</small></div>` : ""}
...
function walletVoucherCard(voucher) {
  ...
  return `<article class="wallet-voucher ${voucher.status}">
    ...
    ${voucher.pixSettlement ? `<div class="pix-redemption-panel complete"><div class="pix-redemption-heading"><span>BENEFIT DELIVERY · PIX SANDBOX</span><b>SIMULATED PAID</b></div><div class="pix-redemption-flow"><i class="done"><b>1</b><span>Voucher validated<small>${voucher.type} · eligible merchant</small></span></i><i class="done"><b>2</b><span>Conversion simulated<small>${voucher.pixSettlement.quote.sourceAmountUsdc} USDC → R$ ${voucher.pixSettlement.payout.amount}</small></span></i><i class="done"><b>3</b><span>Sandbox receipt created<small>${voucher.pixSettlement.merchant.name}</small></span></i></div><div class="pix-receipt">${voucher.pixSettlement.endToEndId ? `<span><b>Sandbox endToEndId</b><code>${voucher.pixSettlement.endToEndId}</code></span>` : `<span><b>Payment reference</b><code>${voucher.pixSettlement.providerReference}</code></span>`}<span><b>Simulated recipient</b>${voucher.pixSettlement.merchant.name}</span><span><b>Simulated payout</b>R$ ${voucher.pixSettlement.payout.amount}<small>Test mode · no real BRL moved</small></span><span><b>Pix audit hash</b><code>${voucher.pixSettlement.auditHash}</code></span></div></div>` : voucher.offRamp ? `<div class="pix-redemption-panel payment-pending"><div class="pix-redemption-heading"><span>PIX OFF-RAMP · ETHERFUSE SANDBOX</span><b>${voucher.offRamp.status === "processing" ? "PROCESSING" : voucher.offRamp.status === "failed" ? "REVIEW REQUIRED" : voucher.offRamp.transactionHash ? "PAYMENT SENT" : "AWAITING APPROVAL"}</b></div><div class="pix-redemption-flow"><i class="done"><b>1</b><span>Voucher validated<small>${voucher.type} · eligible benefit</small></span></i><i class="done"><b>2</b><span>Payment prepared<small>${voucher.offRamp.sourceAmountUsdc} USDC → estimated R$ ${voucher.offRamp.destinationAmountBrl}</small></span></i><i class="${voucher.offRamp.transactionHash ? "done" : ""}"><b>3</b><span>${voucher.offRamp.transactionHash ? "Payment sent" : "Approve payment"}<small>${voucher.offRamp.transactionHash ? "Checking Pix status" : "Wallet confirmation required"}</small></span></i></div>${voucher.offRamp.transactionHash ? `<a href="${voucher.offRamp.explorerUrl}" target="_blank" rel="noopener noreferrer"><span>Redemption transaction</span><code>${voucher.offRamp.transactionHash}</code></a><button type="button" data-wallet-refresh-payment="${voucher.id}">Check Pix status</button>` : `<p>Your Pix payment is ready. Confirm it in your connected wallet to continue.</p><button type="button" data-wallet-approve="${voucher.id}" data-voucher-code="${voucher.code}">Approve Pix payment</button>`}</div>` : voucher.fundedInFull ? `<div class="pix-redemption-panel"><div class="pix-redemption-heading"><span>PIX OFF-RAMP · ETHERFUSE SANDBOX</span><b>1.00 USDC READY</b></div><p>Scan the merchant's Pix QR code to use this funded voucher through the sandbox off-ramp.</p><button type="button" data-wallet-redeem="${voucher.id}" data-voucher-code="${voucher.code}" data-voucher-type="${voucher.type}">Start Pix sandbox delivery</button><small>This is a Pix delivery rail, not a card load. No real BRL will move.</small></div>` : `<div class="pix-redemption-panel"><div class="pix-redemption-heading"><span>VOUCHER FUNDING</span><b>TESTNET FUNDING PENDING</b></div><p>The assistance record exists, but no USDC Testnet funding or card load has been confirmed. Pix sandbox simulation remains a separate demonstration.</p><small>Card delivery requires an issuer or card processor integration.</small></div>`}
    <details class="wallet-proof"><summary><span><b>Audit & funding proof</b><small>${voucher.settlement?.transactionHash ? "Voucher funding verified on Stellar Testnet" : "Integrity record only · funding pending"}</small></span><i>${voucher.settlement?.transactionHash ? "FUNDED" : "PENDING"}</i></summary><div class="wallet-proof-details"><b>Audit hash · ${voucher.auditReceipt?.algorithm} · record integrity</b><code>${voucher.auditReceipt?.hash}</code><b>Timestamp</b><code>${voucher.auditReceipt?.timestamp}</code><b>Voucher funding${voucher.settlement?.onChain ? ` · ${voucher.settlement.amount} ${voucher.settlement.asset} · Stellar Testnet` : ""}</b>${voucher.settlement?.transactionHash ? `<a href="${voucher.settlement.explorerUrl}" target="_blank" rel="noopener noreferrer"><code>${voucher.settlement.transactionHash}</code><span>Open transaction ↗</span></a>` : `<code>Not performed · no USDC Testnet funding confirmed</code>`}<b>Card load</b><code>Not configured · issuer/processor integration required</code><small>${voucher.settlement?.note || ""}</small></div></details>
    ${!voucher.settlement?.transactionHash ? `<button type="button" data-fund-voucher="${voucher.id}">Fund ${voucher.amount} USDC Testnet with Freighter</button><small class="service-payment-note">Non-custodial Testnet funding. Freighter will show the exact amount and destination before you approve.</small>` : ""}
  </article>`;
}
...
walletContent.addEventListener("click", async (event) => {
  const fundButton = event.target.closest("button[data-fund-voucher]");
  if (fundButton) {
    if (!connectedWallet) await connectFreighter();
    if (!connectedWallet) return window.alert("Connect the authorized Freighter wallet on Testnet.");
    fundButton.disabled = true;
    fundButton.textContent = "Preparing voucher funding…";
    try {
      const transactionResponse = await fetch(`/api/vouchers/${fundButton.dataset.fundVoucher}/funding/transaction`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sourceAddress: connectedWallet.address }) });
      const transaction = await transactionResponse.json();
      if (!transactionResponse.ok) throw new Error(transaction.detail || transaction.error || "Voucher funding could not be prepared");
      fundButton.textContent = `Approve ${transaction.amount} USDC in Freighter…`;
      const signed = await window.freighterApi.signTransaction(transaction.unsignedXdr, { networkPassphrase: transaction.networkPassphrase, address: connectedWallet.address });
      if (signed.error || !signed.signedTxXdr) throw new Error(signed.error || "Voucher funding was not approved");
      fundButton.textContent = "Confirming on Stellar Testnet…";
      const submitResponse = await fetch(`/api/vouchers/${fundButton.dataset.fundVoucher}/funding/submit`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sourceAddress: connectedWallet.address, signedXdr: signed.signedTxXdr }) });
      const settlement = await submitResponse.json();
      if (!submitResponse.ok) throw new Error(settlement.detail || settlement.error || "Voucher funding could not be confirmed");
      await openVoucherWallet();
    } catch (error) { fundButton.disabled = false; fundButton.textContent = "Fund voucher with Freighter"; window.alert(error.message); }
    return;
  }
